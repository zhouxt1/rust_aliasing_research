...
|---|---|---|
| `covered` | An existing HB test exercises the same aliasing scenario | 27 |
| `hb_pass` | HB semantics differ from SB/TB (HB allows the program); pattern documented in pass/ | 5 |
...
| `no_dealloc` | Requires deallocation-tracking semantics not implemented in HB | 4 |
| `no_wildcard` | Requires wildcard / exposed-provenance tag support not implemented in HB | 34 |
...
| `no_static` | Requires static/global memory semantics not implemented in HB | 3 |
...
|---|---|---|---|
| `deallocate_against_protector1` | Deallocating heap memory while a strongly-protected `&mut i32` is active triggers a Stacked Borrows violation. | `no_dealloc` | — |
...
| `exposed_only_ro` | A write through a wildcard pointer derived via `expose_provenance` fails when only read-only tags have been exposed. | `no_wildcard` | — |
...
| `illegal_dealloc1` | Deallocating through a pointer whose tag was invalidated by a sibling write is caught by the borrow stack. | `no_dealloc` | — |
...
| `static_memory_modification` | Writing to a static (read-only) allocation via a transmuted mutable reference is detected as UB. | `no_static` | — |
...
| `unescaped_local` | A raw pointer derived via an integer-to-pointer cast cannot write to a local after a fresh reborrow invalidates the exposed tag. | `no_wildcard` | — |
| `unescaped_static` | A raw pointer derived from a reference to a static array element cannot be used via pointer arithmetic to access an adjacent element. | `no_static` | — |
...
| `unknown-bottom-gc` | Checks that the SB GC correctly handles an unknown-bottom (wildcard/exposed-provenance) entry when many SharedReadOnly items are piled on top. | `no_wildcard` | — |
...
| `strongly-protected` | Deallocating memory through a raw pointer derived from a strongly-protected mutable reference (active function-call protector) is forbidden. | `no_dealloc` | — |
...
All tests in this group rely on wildcard / exposed-provenance tag semantics: pointers are created via
integer-to-pointer casts (either `ptr::with_exposed_provenance` or `addr as *mut _`) and TB tracks
them with a special WILD root whose child-identification rules differ fundamentally from ordinary
reborrow trees.  HB does not implement wildcard-tag provenance tracking, so none of these tests can
be ported.

Affected tests (28): `cross_tree_from_main`, `cross_tree_update_main`,
...
`protector_release`, `protector_release2`, `single_exposed_disable`, `single_exposed_foreign`,
`single_exposed_local`, `single_exposed_only_ro`, `strongly_protected_wildcard`,
...
| `sb_fails` | Mutable reborrows without actual writes do not invalidate sibling raw pointers (SB fails, TB/HB pass), across four aliasing patterns. | `covered` | `sb_fnentry_doesnt_invalidate_raw.rs`, `hb_pass_invalid_mut_raw_read.rs`, `hb_return_invalid_mut_raw_read.rs` (portable sub-modules; `static_memory_modification` sub-module is `no_static`) |
